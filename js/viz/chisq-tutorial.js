...
function renderChisqChoiceGuide(el) {
  ...
  el.innerHTML = card(title, `
    <div style="${grid}">
      <div style="${boxStyle}">
        <span style="${badge}">独立 2×2 表</span>
        <div style="font-weight:700;color:#0f172a;margin:8px 0;">两组独立样本率比较</div>
        <div style="${muted}">常用 Pearson χ²；理论频数偏小时考虑连续性校正或 Fisher 确切概率法。</div>
      </div>
      <div style="${boxStyle}">
        <span style="${badge}">配对 2×2 表</span>
        <div style="font-weight:700;color:#0f172a;margin:8px 0;">同一对象两次分类结果</div>
        <div style="${muted}">使用 McNemar 检验，重点看两个不一致格子 b 和 c，而不是普通独立样本 χ²。</div>
      </div>
      <div style="${boxStyle}">
        <span style="${badge}">小样本 2×2 表</span>
        <div style="font-weight:700;color:#0f172a;margin:8px 0;">理论频数过小</div>
        <div style="${muted}">优先考虑 Fisher 确切概率法，尤其是样本量小或某些理论频数明显小于 5 时。</div>
      </div>
      <div style="${boxStyle}">
        <span style="${badge}">R×C 表</span>
        <div style="font-weight:700;color:#0f172a;margin:8px 0;">多组率或构成比比较</div>
        <div style="${muted}">总体 χ² 检验只说明分布不全相同；需要结合期望频数和残差判断主要贡献格子。</div>
      </div>
    </div>
  `);
}
...
function renderChisq2x2(el) {
  ...
  const init = {
    a: Number(el.dataset.a || 43),
    b: Number(el.dataset.b || 157),
    c: Number(el.dataset.c || 26),
    d: Number(el.dataset.d || 174)
  };
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px;align-items:start;">
      <div style="${boxStyle}">
        ...
        <table style="border-collapse:collapse;width:100%;margin-bottom:10px;">
          <thead><tr><th></th><th>阳性</th><th>阴性</th></tr></thead>
          <tbody>
  ...
  function read(cell) {
    return Number(document.getElementById(`${id}-${cell}`).value);
  }

  function calc() {
    const obs = [[read('a'), read('b')], [read('c'), read('d')]];
    if (obs.flat().some(v => !Number.isInteger(v) || v < 0) || sum(obs.flat()) === 0) {
      document.getElementById(`${id}-summary`).innerHTML = '<div style="color:#b91c1c;">请输入非负整数频数，且总例数不能为 0。</div>';
      return;
    }
    ...
    if (rowTotals.some(v => v === 0) || colTotals.some(v => v === 0)) {
      document.getElementById(`${id}-summary`).innerHTML = '<div style="color:#b91c1c;">每一行、每一列都至少需要有一个观察值。</div>';
      return;
    }
    ...
    let chisq = 0;
    let yates = 0;
    const contrib = obs.map((row, i) => row.map((o, j) => {
      const c = (o - exp[i][j]) ** 2 / exp[i][j];
      chisq += c;
      yates += (Math.max(0, Math.abs(o - exp[i][j]) - 0.5) ** 2) / exp[i][j];
      return c;
    }));
    ...
    document.getElementById(`${id}-summary`).innerHTML = `
      ...
      <div style="${muted}">OR ≈ ${fmt(or)}。</div>
    `;

    const rows = ['组1', '组2'];
    const cols = ['阳性', '阴性'];
    const tableRows = obs.map((row, i) => row.map((o, j) => `
      <td style="padding:8px;border:1px solid #cbd5e1;text-align:center;background:#fff;">
        <div><strong>O=${o}</strong></div>
        <div style="font-size:12px;color:#64748b;">E=${fmt(exp[i][j])}</div>
        <div style="font-size:12px;color:#64748b;">贡献=${fmt(contrib[i][j])}</div>
      </td>`).join(''));
    document.getElementById(`${id}-details`).innerHTML = `
      <div style="${boxStyle}">
        <div style="font-weight:700;color:#0f172a;margin-bottom:8px;">观察频数 O、理论频数 E 与 χ² 贡献</div>
        <table style="border-collapse:collapse;width:100%;">
          <thead><tr><th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;"></th>${cols.map(c => `<th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;">${c}</th>`).join('')}</tr></thead>
          <tbody>${rows.map((r, i) => `<tr><th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;text-align:left;">${r}</th>${tableRows[i]}</tr>`).join('')}</tbody>
        </table>
      </div>`;
  }
...
function exactMcNemarP(b, c) {
  ...
  let lower = 0;
  for (let i = 0; i <= x; i++) lower += binomPMF(i, n, 0.5);
...
function renderMcNemarGuide(el) {
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px;align-items:start;">
      <div style="${boxStyle}">
        <table style="border-collapse:collapse;width:100%;margin-bottom:10px;">
          ...
          <tbody>
            <tr><th style="text-align:left;">方法A +</th><td><input id="${id}-a" type="number" min="0" step="1" value="${init.a}" style="width:80px;padding:5px;"></td><td><input id="${id}-b" type="number" min="0" step="1" value="${init.b}" style="width:80px;padding:5px;"></td></tr>
            <tr><th style="text-align:left;">方法A −</th><td><input id="${id}-c" type="number" min="0" step="1" value="${init.c}" style="width:80px;padding:5px;"></td><td><input id="${id}-d" type="number" min="0" step="1" value="${init.d}" style="width:80px;padding:5px;"></td></tr>
          </tbody>
        </table>
      </div>
  ...
  function calc() {
    ...
    const c = read('c');
    const d = read('d');
    if ([a, b, c, d].some(v => !Number.isInteger(v) || v < 0)) {
      document.getElementById(`${id}-result`).innerHTML = '<div style="color:#b91c1c;">请输入非负整数频数。</div>';
      return;
    }
    const discordant = b + c;
    const stat = discordant > 0 ? ((b - c) ** 2) / discordant : NaN;
    const yates = discordant > 0 ? ((Math.abs(b - c) - 1) ** 2) / discordant : NaN;
    const p = chiSquareP(stat, 1);
    const py = chiSquareP(yates, 1);
    const exact = exactMcNemarP(b, c);
    document.getElementById(`${id}-result`).innerHTML = `
      <div style="font-weight:700;color:#0f172a;margin-bottom:8px;">读图重点</div>
      <div style="${muted}">一致格子 a=${a}、d=${d} 不决定 McNemar 检验统计量；核心是不一致格子 b=${b} 与 c=${c} 是否对称。</div>
      <div style="${muted}">不校正 χ² = ${fmt(stat)}，P = ${fmtP(p)}。</div>
      <div style="${muted}">连续性校正 χ² = ${fmt(yates)}，P = ${fmtP(py)}。</div>
      <div style="${muted}">精确二项检验 P = ${fmtP(exact)}。${discordant < 25 ? '<strong style="color:#b45309;">不一致对子较少时更建议报告精确 P 值。</strong>' : ''}</div>
    `;
  }
...
function renderChisqResidualHeatmap(el) {
  ...
  let chisq = 0;
  const residuals = observed.map((row, i) => row.map((o, j) => {
  ...
  function bg(r) {
    const a = Math.min(0.85, Math.abs(r) / 3 * 0.85);
    return r >= 0 ? `rgba(37,99,235,${a})` : `rgba(220,38,38,${a})`;
  }

  el.innerHTML = card(title, `
    <div style="${boxStyle};margin-bottom:12px;${muted}">
      总体 χ² = ${fmt(chisq)}，df = ${df}，P = ${fmtP(p)}。总体检验说明分类分布是否不全相同；标准化残差用于判断哪些格子偏离期望最多。
    </div>
    <div style="overflow-x:auto;">
      <table style="border-collapse:collapse;width:100%;min-width:520px;">
        <thead><tr><th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;"></th><th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;">结局1</th><th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;">结局2</th><th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;">结局3</th></tr></thead>
        <tbody>
          ${observed.map((row, i) => `<tr><th style="padding:8px;border:1px solid #cbd5e1;background:#f1f5f9;text-align:left;">组${i + 1}</th>${row.map((o, j) => {
            const r = residuals[i][j];
            return `<td style="padding:8px;border:1px solid #cbd5e1;text-align:center;background:${bg(r)};color:${Math.abs(r) > 1.6 ? '#fff' : '#0f172a'};"><div><strong>O=${o}</strong></div><div style="font-size:12px;">E=${fmt(expected[i][j])}</div><div style="font-size:12px;">残差=${fmt(r)}</div></td>`;
          }).join('')}</tr>`).join('')}
        </tbody>
      </table>
    </div>
    <div style="${boxStyle};margin-top:12px;${muted}">
      残差为正：观察频数高于理论频数；残差为负：观察频数低于理论频数。绝对值越大，对总体 χ² 的贡献越大。
    </div>
  `);
}
