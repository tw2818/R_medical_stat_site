...
function card(title, body) {
  return `<div class="viz-card"><div class="viz-header"><span>📊 ${title}</span></div><div style="padding:14px;background:var(--v2-bg-elevated);">${body}</div></div>`;
}
...
function renderCorrelationChoiceGuide(el) {
  ...
  el.innerHTML = card(title, `<div style="${grid}">
    <div style="${boxStyle}"><span style="${badge}">直线回归</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">有明确 X 和 Y</div><div style="${muted}">用于建立 Y 随 X 改变的预测/解释模型。重点是斜率、截距、残差和模型适用范围。</div></div>
    <div style="${boxStyle}"><span style="${badge}">Pearson 相关</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">两个连续变量的线性关联</div><div style="${muted}">相关系数 r 描述线性关联方向和强度。它不区分自变量/因变量，也不等于因果效应。</div></div>
    <div style="${boxStyle}"><span style="${badge}">Spearman 秩相关</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">单调关系或等级资料</div><div style="${muted}">先把原始值转为秩，再对秩计算相关。适合偏态、异常值影响较大或等级变量。</div></div>
    <div style="${boxStyle}"><span style="${badge}">曲线拟合</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">关系明显非线性</div><div style="${muted}">用于描述或预测非线性趋势。应警惕过拟合和外推风险，不能因拟合好就推断机制成立。</div></div>
  </div>`);
}
...
function renderRegressionCorrelationGuide(el) {
  ...
  function calc() {
    ...
    summary.innerHTML = `<div style="font-weight:700;color:var(--v2-fg);margin-bottom:8px;">结果解释</div><div style="${muted}"><strong>回归方程</strong>：ŷ = ${fmt(fit.intercept, 3)} + ${fmt(fit.slope, 3)}X。</div><div style="${muted}"><strong>Pearson r</strong> = ${fmt(fit.r, 3)}，表示线性关联方向和强度。</div><div style="${muted}"><strong>R²</strong> = ${fmt(fit.r2, 3)}。在简单线性回归中 R² = r²，但 R² 是模型解释变异比例，r 是相关系数。</div><div style="${muted}">相关不等于因果；回归斜率也需要结合研究设计和混杂控制解释。</div>`;
    draw(x, y, fit);
  }
  function draw(x, y, fit) {
    ...
    ctx.fillStyle = 'var(--v2-bg-elevated)'; ctx.fillRect(0, 0, W, H);
    ctx.strokeStyle = 'var(--v2-fg-secondary)'; ctx.lineWidth = 1.4;
    ...
    ctx.fillStyle = 'var(--v2-fg)'; ctx.font = '12px sans-serif'; ctx.textAlign = 'center'; ctx.fillText('X', pad.l + (W - pad.l - pad.r) / 2, H - 10);
...
function renderSpearmanRankGuide(el) {
  ...
  function calc() {
    ...
    summary.innerHTML = `<div style="font-weight:700;color:var(--v2-fg);margin-bottom:8px;">读图重点</div><div style="${muted}">Spearman 相关就是对两个变量的秩计算 Pearson 相关。</div><div style="${muted}"><strong>Spearman ρ</strong> = ${fmt(rho, 3)}。</div><div style="${muted}">${rx.tieGroups || ry.tieGroups ? `存在结值：X 有 ${rx.tieGroups} 组，Y 有 ${ry.tieGroups} 组；使用平均秩。` : '无结值。'}</div>`;
...
function renderSlopeCompareGuide(el) {
  ...
  el.innerHTML = card(title, `<div style="${grid}">
    <div style="${boxStyle}"><span style="${badge}">模型</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">y ~ x * group</div><div style="${muted}">展开后为 y = β0 + β1x + β2group + β3(x×group)。其中 β3 是斜率差异。</div></div>
    <div style="${boxStyle}"><span style="${badge}">先看斜率</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">β3 是否显著</div><div style="${muted}">若交互项显著，说明两组 x 与 y 的关系强度不同，不宜只比较截距或合并成一条线。</div></div>
    <div style="${boxStyle}"><span style="${badge}">再看截距</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">β2 的解释依赖 x=0</div><div style="${muted}">截距差异表示在 x=0 时两组均值差异。若 x=0 没有实际意义，可先中心化 x。</div></div>
    <div style="${boxStyle}"><span style="${badge}">报告</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">图 + 模型系数</div><div style="${muted}">建议画分组散点和两条拟合线，同时报告交互项估计值、置信区间和 P 值。</div></div>
  </div>`);
}
...
function renderCurveFitGuide(el) {
  ...
  el.innerHTML = card(title, `<div style="${grid}">
    <div style="${boxStyle}"><span style="${badge}">线性模型</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">y = β0 + β1x</div><div style="${muted}">适合近似直线关系。残差图若显示弯曲结构，说明线性假设可能不足。</div></div>
    <div style="${boxStyle}"><span style="${badge}">二次/多项式</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">加入 x²、x³ 等项</div><div style="${muted}">可描述弯曲趋势，但高阶项容易过拟合；应结合散点图、残差图和外部合理性。</div></div>
    <div style="${boxStyle}"><span style="${badge}">拟合好 ≠ 机制真</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">警惕解释过度</div><div style="${muted}">曲线拟合主要是描述/预测工具。观察性资料中，非线性关联仍可能来自混杂或选择偏倚。</div></div>
    <div style="${boxStyle}"><span style="${badge}">外推风险</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">不要超出数据范围太远</div><div style="${muted}">曲线在观测范围内拟合良好，不代表在低于最小值或高于最大值的区域仍有效。</div></div>
  </div>`);
}
