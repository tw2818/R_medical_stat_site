...
function renderNonparametricChoiceGuide(el) {
  ...
  el.innerHTML = card(title, `<div style="${grid}">
    <div style="${boxStyle}"><span style="${badge}">配对设计</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">Wilcoxon 符号秩检验</div><div style="${muted}">先计算每对差值，去掉 0 差值，再按差值绝对值排序并加回正负号。</div></div>
    <div style="${boxStyle}"><span style="${badge}">两独立组</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">Wilcoxon 秩和检验 / Mann-Whitney U</div><div style="${muted}">把两组合并排序，再比较两组秩和。它主要比较分布位置，不是直接比较均数。</div></div>
    <div style="${boxStyle}"><span style="${badge}">多个独立组</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">Kruskal-Wallis H 检验</div><div style="${muted}">多组资料合并排序，检验各组分布位置是否不全相同。显著后需事后多重比较。</div></div>
    <div style="${boxStyle}"><span style="${badge}">注意</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">不是万能替代</div><div style="${muted}">非参数检验减少对正态分布的依赖，但仍要求研究设计正确；配对和独立样本不能混用。</div></div>
  </div>`);
}
...
function renderSignedRankGuide(el) {
  ...
  function calc() {
    ...
    document.getElementById(`${id}-summary`).innerHTML = `<div style="font-weight:700;color:var(--v2-fg);margin-bottom:8px;">读图重点</div><div style="${muted}">原始配对数 = ${b.length}；去掉 0 差值后 n = ${rows.length}。</div><div style="${muted}">正秩和 W+ = ${fmt(wPlus, 1)}；负秩和 W− = ${fmt(wMinus, 1)}。</div><div style="${muted}">${tieGroups ? `存在 ${tieGroups} 组结值，R 会使用平均秩并可能改用近似 P 值。` : '无结值。'}</div>`;
...
function renderRankSumGuide(el) {
  ...
  function calc() {
    ...
    document.getElementById(`${id}-summary`).innerHTML = `<div style="font-weight:700;color:var(--v2-fg);margin-bottom:8px;">读图重点</div><div style="${muted}">两组样本合并后一起排序，再比较各组秩和。</div><div style="${muted}">组1秩和 = ${fmt(r1, 1)}；组2秩和 = ${fmt(r2, 1)}；U = ${fmt(Math.min(u1, u2), 1)}。</div><div style="${muted}">${tieGroups ? `存在 ${tieGroups} 组结值，精确 P 值可能不可用。` : '无结值。'}</div>`;
...
function renderKruskalWallisGuide(el) {
  ...
  el.innerHTML = card(title, `<div style="${grid}">
    <div style="${boxStyle}"><span style="${badge}">第一步</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">合并所有观测值排序</div><div style="${muted}">不在每个组内部排序，而是把所有组放在一起赋秩；结值使用平均秩。</div></div>
    <div style="${boxStyle}"><span style="${badge}">第二步</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">比较各组平均秩</div><div style="${muted}">若各组来自相同分布，平均秩应大致接近；偏离越大，H 统计量越大。</div></div>
    <div style="${boxStyle}"><span style="${badge}">第三步</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">总体检验后再做多重比较</div><div style="${muted}">H 检验显著只说明多组不全相同，不能直接说明哪两组不同；需 Dunn 检验或 pairwise Wilcoxon 并校正 P 值。</div></div>
    <div style="${boxStyle}"><span style="${badge}">解释</span><div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">比较分布位置而非均数</div><div style="${muted}">当各组分布形状相近时，可近似解释为中位数或位置差异；分布形状不同则解释更谨慎。</div></div>
  </div>`);
}
