  ...
  el.innerHTML = card(title, `
    <div style="${grid3}">
      <div style="${boxStyle}">
        ...
        <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">固定 n 中的阳性数</div>
        <div style="${muted}">适合成功/失败、阳性/阴性这类二分类结局。核心问题通常是率的估计和率的比较。</div>
        <div style="margin-top:10px;font-size:14px;color:var(--v2-fg);"><strong>X ~ B(n, p)</strong></div>
      </div>
      <div style="${boxStyle}">
        <span style="${badge}">泊松分布</span>
        <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">固定观察量内的事件数</div>
        <div style="${muted}">适合单位时间、单位面积、人年等观察量中的事件次数。比较时应看事件率，而不是只看原始事件数。</div>
        <div style="margin-top:10px;font-size:14px;color:var(--v2-fg);"><strong>X ~ Poisson(λ)</strong></div>
      </div>
      <div style="${boxStyle}">
        <span style="${badge}">负二项分布</span>
        <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">过度离散的计数资料</div>
        <div style="${muted}">当计数资料的方差明显大于均数时，泊松分布的“均数 = 方差”假设可能不合适，可考虑负二项分布。</div>
        <div style="margin-top:10px;font-size:14px;color:var(--v2-fg);"><strong>Var(X) &gt; E(X)</strong></div>
      </div>
    </div>
    ...
    ? `
      <div style="${grid3}">
        <div style="${boxStyle}">
          ...
          <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">事件数</div>
          <div style="${muted}">在固定观察时间、观察面积或人年内记录到的事件次数。</div>
        </div>
        <div style="${boxStyle}">
          <span style="${badge}">λ</span>
          <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">期望事件数</div>
          <div style="${muted}">单位观察量内的平均事件数；在标准化观察量后，也可理解为事件率参数。</div>
        </div>
        <div style="${boxStyle}">
          <span style="${badge}">注意</span>
          <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">均数 ≠ 连续变量均数</div>
    ...
    : `
      <div style="${grid3}">
        <div style="${boxStyle}">
          ...
          <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">固定试验次数 / 样本量</div>
          <div style="${muted}">例如调查 100 人，n = 100。</div>
        </div>
        <div style="${boxStyle}">
          <span style="${badge}">p</span>
          <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">每次成功概率 / 总体率</div>
          <div style="${muted}">例如总体阳性率、治愈率、患病率。</div>
        </div>
        <div style="${boxStyle}">
          <span style="${badge}">X</span>
          <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">成功数 / 阳性数</div>
...
function renderNegativeBinomialGuide(el) {
  ...
  el.innerHTML = card(title, `
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;align-items:stretch;">
      <div style="${boxStyle}">
        <span style="${badge}">泊松分布</span>
        <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">均数 = 方差</div>
        <div style="font-size:18px;color:var(--v2-fg);margin:8px 0;"><strong>E(X)=λ, Var(X)=λ</strong></div>
        <div style="${muted}">适合事件发生相对独立且离散程度与均数接近的计数资料。</div>
      </div>
      <div style="${boxStyle}">
        <span style="${badge}">负二项分布</span>
        <div style="font-weight:700;color:var(--v2-fg);margin:8px 0;">方差大于均数</div>
        <div style="font-size:18px;color:var(--v2-fg);margin:8px 0;"><strong>Var(X) &gt; E(X)</strong></div>
