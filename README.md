...
### 2026-04-22 — 统一第二章部分小节导语风格，并顺滑方差齐性结论表述
- **问题章节**：`1002-anova.html`
- **修复内容**：在章节渲染时统一第二章若干设计类型小节的导语风格，并将方差齐性检验结尾处较拧的双重否定表述改成更顺的结论性表达
- **原因**：第二章后期剩余问题已主要集中在风格统一和阅读顺滑度，而不是统计内容本身
- **方式**：沿用小范围 post-load text refinement，不直接重写大型 Quarto 导出 HTML

### 2026-04-22 — 补强第二章中两处教学性说明
- **问题章节**：`1002-anova.html`
- **修复内容**：在章节渲染时为两阶段交叉设计一节补充 `phase`、`type`、`testid` 的简短模型解释，并将 SNK-q 检验一节“与课本不一样”的说明改写为更完整的实现差异提示
- **原因**：第二章剩余问题主要已从“硬错误”转为“解释不够”，这轮优先补齐最容易让初学者卡住的两处说明
- **方式**：沿用小范围 post-load text refinement，不直接重写大型 Quarto 导出 HTML

### 2026-04-22 — 修正第二章中两处 ANOVA 结果说明的正文错误
- **问题章节**：`1002-anova.html`
- **修复内容**：在章节渲染时修正随机区组设计一节中的区组间 F 值笔误（`5.798 → 5.978`），并修正拉丁方设计一节中坏掉的“组内离均差平方和”数值文案（改为 `683.2`）
- **原因**：这两处属于正文层面的确定性错误，会影响读者对 ANOVA 输出的核对
- **方式**：沿用小范围 post-load text correction，不直接重写大型 Quarto 导出 HTML

### 2026-04-22 — 打磨第一章的导语、结果说明与标题措辞
- **问题章节**：`1001-ttest.html`
- **修复内容**：在章节渲染时对第一章的开场导语、分布探索提示语、`t.test()` 说明、单样本结果解释、配对样本数据说明，以及 `1.4` 标题做小范围措辞优化
- **原因**：前面几轮已经把组件位置、数据一致性和明显错误清理掉了，这一轮主要把文案改得更严谨、更顺、更贴近教学节奏
- **方式**：沿用小范围 post-load text refinement，不直接重写大型 Quarto 导出 HTML

### 2026-04-22 — 移除第一章开头与 t 检验主线不匹配的三组箱线图
- **问题章节**：`1001-ttest.html`
- **修复内容**：在章节渲染时移除开头“正态性检验与分布探索”部分中的 `box` 组件
- **原因**：该组件展示的是三组血压比较，更贴近第二章方差分析语境，不适合作为第一章 t 检验的开场组件
- **方式**：沿用小范围 post-load cleanup，不直接重写大型 Quarto 导出 HTML

### 2026-04-22 — 统一第一章配对样本组件与正文例题数据
- **问题章节**：`1001-ttest.html`
- **修复内容**：将配对散点图和 Bland-Altman 组件的数据改为与正文例 3-6 完全一致的 10 对 `x1/x2` 数据
- **原因**：之前组件使用的是另一组 15 对数据，和正文的配对 t 检验示例不一致，会削弱章节内容与交互演示之间的可信度
- **方式**：直接修正章节中的组件 `data-*` 属性，使可视化、原始数据和检验结果保持同源

### 2026-04-22 — 移除第一章配对 t 检验中错误的 formula 演示段落
- **问题章节**：`1001-ttest.html`
- **修复内容**：在章节渲染时移除配对样本 t 检验部分那段错误的 `formula` 演示、对应长表转换示例、误导性的“结果也是一模一样的”结论，以及后续围绕该示例的版本警告
- **原因**：该段展示出的结果实际对应两样本 t 检验，不是前文的配对 t 检验，继续保留会误导读者
- **方式**：沿用小范围 post-load cleanup，不直接重写大型 Quarto 导出 HTML
...
- **原因**：第一章中的分布图、P 值组件和 t 检验计算器在手机端最容易出现内容挤压、裁切或不完整显示
- **方式**：优先通过 CSS 响应式规则修复，不改动组件计算逻辑

### 2026-04-22 — 移除方差分析章节中多重比较部分重复的 ANOVA 组件
- **问题章节**：`1002-anova.html`
- **修复内容**：在章节渲染时移除 `2.5 多重比较` 开头重复出现的 `anova` 组件，仅保留 `2.1` 中用于解释总体 ANOVA 的核心 `anova` 与 `fdist` 组件
- **原因**：同一章内重复使用同一组 4 组均值数据会造成轻度视觉重复；多重比较部分已有足够的后续图形和输出支撑理解
- **方式**：沿用小范围 post-load cleanup，不直接重写大型 Quarto 导出 HTML

### 2026-04-22 — 调整 t 检验章节中重复的 P 值组件位置
- **问题章节**：`1001-ttest.html`
- **修复内容**：在章节渲染时移除前面过早出现的固定版 `pvalue` 组件，仅保留结果解释后的交互版 `pvalue` 与 `ttest` 计算器
- **原因**：按教学顺序，应先展示代码和检验结果，再通过 P 值可视化和计算器帮助读者理解；前置的固定版组件会造成重复感
- **方式**：沿用小范围 post-load cleanup，不直接重写大型 Quarto 导出 HTML

### 2026-04-22 — 移除多项式拟合章节中不够贴题的剂量反应组件
- **问题章节**：`1039-nonlinear.html`
- **修复内容**：在章节渲染时移除 `dose` 组件
- **原因**：该组件展示的是剂量-反应 / Sigmoid 曲线，和本章“多项式拟合”的方法主线不够贴合
- **方式**：沿用小范围 post-load cleanup，不直接重写大型 Quarto 导出 HTML
